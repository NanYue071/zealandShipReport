...
class NorthPortSpider(Spider):
            ...
            for table in tables:
                ...
                for row in rows:
                    ...
                    if re.search(r"logs", cargo, re.IGNORECASE):
                        ...
                        yield item
        next_url = 'https://northport.co.nz/departedpage/allshippingmovements'
    ...
    def parse_next_page(self, response):
        ...
        # 继续处理新页面的数据
        # 例如，假设我们从新页面上抓取某些信息
        tables = response.xpath('.//table[@class="views-table cols-13 table table-condensed table-0 table-0 table-0"]')
        if tables is not None:
            for table in tables:
                status = table.xpath('.//caption/text()').get()
                rows = table.xpath('.//tbody/tr')
