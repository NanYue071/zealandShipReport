...
from zealandShipReport.utils.cache_utils import Cache
from zealandShipReport.utils.thread_pool import ThreadPool
...
def get_ips(pages=1, refresh=True):
    """
...
def crawl_quick(page=1):
    ...
    assert 1 <= page <= 10, '页数有效范  围为（1 - 10）'
    validate_ips = []
    for i in range(page):
        response = requests.get(url=PROXY_IP_QUICK_URL % str(i + 1), headers=PROXY_IP_QUICK_HEADERS)
        if response.status_code == 200:
            # 解析页面元素
            soap = BeautifulSoup(response.text, 'html.parser')
            scripts = soap.find_all('script')
            for script in scripts:
                for line in script:
                    scriptString = str(line)
                    if 'fpsList' in scriptString:
                        pattern = re.compile(r'fpsList = .*?;')
                        ip_table = re.search(pattern, scriptString)
            ips = []
            ip_table = str(ip_table.group()).split("=")[1].strip().strip('[').strip(']').split('},')
            # 获取当前页的所有 ip 信息
            for data in ip_table:
                data = data.strip().strip(';').strip(']').strip('}')
                item = json.loads(data + '}')
                info = {}
                ip, port, area, proxy_type, protocol, alive_time, check_time = item['ip'], item['port'], item['location'], '高匿名', 'HTTP', '', item['last_check_time']
